...
{
    public class UserInfirmerieController : Controller
    {
        OracleDataReader dr;
        ...
        public IActionResult ValiderFreq(string id, double temp, double tamax, double tamin, double puls, double poids, double taille, double pc, double album, double glycemie)
        {
            ...
            {
                CommandText = "UPDATE FREQMALA_JDE SET TEMP = '" + temp + "' , TAMAX = '" + tamax + "' , TAMIN ='" + tamin + "' , PULLS = '" + puls + "' , POIDS = '" + poids + "', PC = '" + pc + "' , ALBUM = '" + album + "' , GLICEMIE = '" + glycemie + "' , HACMS = TO_DATE(SYSDATE, 'HH24:MI:SS') WHERE FREQMALA ='" + id + "'",
                Connection = conn,
